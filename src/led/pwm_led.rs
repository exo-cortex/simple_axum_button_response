...
use rppal::pwm::{Channel::Pwm0, Polarity, Pwm};
...
pub fn drive_pwm_led(
    ...
    let mut frequency = PWM_CYCLE_FREQUENCY_HZ;
    let mut duty_cycle = 0.0;
    let mut phase = 0.0;
    let mut next_pwm_event: Option<Instant> = None;
    // let mut intensity_phase: f64 = 0.0;
    // let mut breath_direction = true;
    ...
    loop {
...

        {
            ...
            let mut state = pwm_state.lock().expect("could not lock pwm_state");
        ...
        match led_state {
            ...
            PwmLedMode::Breath { period_ms } => {
                ...
                duty_cycle = map_triangle(value * value);
